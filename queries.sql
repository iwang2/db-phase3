...
#selected available card types and dates
...
SELECT first_name, last_name
FROM USER
WHERE first_name = "?" AND last_name = "?"; #HOW TO CHECK IF IT IS AN ADMIN?


##REVIEW PASSENGER REVIEWS SCREEN##
#admin review passengers review screen pending reviews
SELECT passenger_id, station_name, shopping, connection_speed, comment
FROM REVIEW
...
#removing rejected/approved reviews from pending????
...
#how to check if that line already has a line with that order number???


##ADD LINE SCREEN##
#how to ensure the line name is unique
#how to prevent having two stations with the same order number
...
##how to add into admin add line and admin add station???
